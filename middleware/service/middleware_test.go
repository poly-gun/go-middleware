...
func Test(t *testing.T) {
	...
	t.Run("Middleware", func(t *testing.T) {
		t.Run("Successful-Default-Service-Headers", func(t *testing.T) {
			const k = "X-Service-Name"
			const v = "Test-Service-1"

			server := httptest.NewServer(service.New().Settings(func(options *service.Options) {
				options.Name = v
			}).Handler(handler))
		...
		t.Run("Successful-Custom-Service-Headers", func(t *testing.T) {
			const k = "X-Test-Service-Name"
			const v = "Test-Service-2"
		...
		t.Run("Emitted-Default-Warning", func(t *testing.T) {
			...
			server := httptest.NewServer(service.New().Handler(handler))
		...
		t.Run("No-Emitted-Warning", func(t *testing.T) {
			...
			server := httptest.NewServer(service.New().Settings(func(o *service.Options) {
				o.Warnings = false
			}).Handler(handler))
	...
	t.Run("Context", func(t *testing.T) {
		...
		t.Run("User-Specified-Value", func(t *testing.T) {
			...
			if value != "Test-Service" {
				t.Errorf("Unexpected Context Value Received: %v, Expected: %s", value, "Test-Service")
			}
